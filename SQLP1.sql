...
WITH MaxDeathRatio AS (
SELECT 
location,
MAX(total_deaths/population) AS maxDeathRatio
FROM PortfolioProject..CovidDeaths
GROUP BY location
)
SELECT DISTINCT
...
cd.population,
cd.total_deaths,
(cd.total_deaths/cd.population)*100 AS deathsPerPop
FROM PortfolioProject..CovidDeaths cd
INNER JOIN  MaxDeathRatio mdr ON cd.location = mdr.location
AND cd.total_deaths/cd.population = mdr.maxDeathRatio
...
SELECT location,MAX(cast(total_deaths as int)) as DeathByContinent
FROM PortfolioProject..CovidDeaths
